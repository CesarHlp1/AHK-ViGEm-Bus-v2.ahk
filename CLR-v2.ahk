...
CLR_LoadLibrary(AssemblyName, AppDomain := 0) {
    ...
    ; Si es una ruta completa, priorizamos LoadFrom
    if InStr(AssemblyName, "\") || InStr(AssemblyName, "/") {
        try {
            args := ComObjArray(0xC, 1)
            args[0] := AssemblyName
            typeofAssembly := AppDomain.GetType().Assembly.GetType()
            static null := ComValue(0xD, 0)
            assembly := typeofAssembly.InvokeMember_3("LoadFrom", 0x158, null, null, args)
            if assembly
                return assembly
        } catch {
    ...
    ; Intentos estándar
    try assembly := AppDomain.Load_2(AssemblyName)
    if assembly
        return assembly
    ...
    try {
        ...
        args[0] := AssemblyName
        typeofAssembly := AppDomain.GetType().Assembly.GetType()
        assembly := typeofAssembly.InvokeMember_3("LoadWithPartialName", 0x158, null, null, args)
        if assembly
            return assembly
    } catch {
    }

    throw Error("No se pudo cargar el ensamblado: " AssemblyName "`nVerifica que ViGEmWrapper.dll y ViGEmClient.dll estén en la carpeta y sean compatibles.")
}

; El resto de funciones CLR (CLR_CreateObject, CLR_GetDefaultDomain, CLR_Start, etc.) déjalas como están
; Solo asegúrate de que ComValue(0xD, 0) esté usado en lugar de ComObject(13,0) en todos lados
...
CLR_CreateObject(Assembly, TypeName, Args*)
{
	...
	static Array_Empty := ComObjArray(0xC,0), null := ComValue(0xD, 0)
